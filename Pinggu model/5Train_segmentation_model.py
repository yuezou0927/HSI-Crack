...
###############################################################################
data_dir = 'crack_data_Pinggu/train/traindata/0' 
label_dir = 'Pinggu model/1111result/1111pse-label'
...
class HyperspectralDataset(Dataset):
    def __init__(self, img_dir, label_dir):
        ...
        for label_file in self.label_files:
            ...
            img_file = f"{base_name}.npy"
            img_path = os.path.join(img_dir, img_file)
            if os.path.exists(img_path):
                self.img_files.append(img_file)
            else:
                raise FileNotFoundError(f"Image file {img_path} not found for label file {label_file}.")
